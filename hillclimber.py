...
class HillClimber:
    ...
    def get_target_shape(self, timesteps=1):
        """
    ...
    def compress_to_8x8(self, binary_array):
        """Compresses a binary array to 8x8"""
        ...
        for i in range(0, binary_array.shape[0], 8):
            for j in range(0, binary_array.shape[1], 8):
                ...
                compressed_array[i//8, j//8] = np.sum(block) > 32

        return compressed_array
                                            
    # def evaluate_phenotypes(self, phenotypes, timesteps=5):
    #     """Score a set of phenotypes generated by the simulate function."""
    #     resolution = 64 // self.layers[self.base_layer]['res']
    #     print('Evaluation resolution: ', resolution)
    #     target = self.get_target_shape(timesteps, target_size=resolution)
