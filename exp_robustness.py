...
def measure_neutral_neighborhood_percentage(exp, solution):
    solution_genome_size = len(np.transpose(np.nonzero(solution.state_genotype)))
    neutral_percent_sum = 0
    for j in range(solution_genome_size):
        ne = NeutralEngine(exp, solution, solution.state_genotype, mutate_layers=None, mutate_param=j)
        ne.run(1)
        neutral_percent_sum += ne.neutral_percent
        print(f"Param {j} neutral percent: {ne.neutral_percent}")

    return neutral_percent_sum / solution_genome_size

parser = argparse.ArgumentParser()
parser.add_argument('exp_file', type=str, help='Experiment File')
args = parser.parse_args()

# Get the directory containing the experiment file
exp_dir = os.path.dirname(args.exp_file)

# Create a results file in the same directory
results_file = os.path.join(exp_dir, "robustness_results.txt")
with open(results_file, 'w') as f:
    f.write("Filename,Fitness,Neutral_Neighborhood_Percentage\n")

# Iterate through all files in the directory
for filename in os.listdir(exp_dir):
    if not filename.endswith('.pkl'):
        continue
    exp_file = os.path.join(exp_dir, filename)
    print(f"\nProcessing {filename}...")

    exp = get_experiment(exp_file)
    best = exp.best_solution()

    best.neutral_counter = 0
    best.genotype_distance = 0

    neutral_percent = measure_neutral_neighborhood_percentage(exp, best)

    print('Top fitness: ', best.fitness)
    print(f'Neutral neighborhood percentage: {neutral_percent}')

    # Write results to the file
    with open(results_file, 'a') as f:
        f.write(f"{filename},{best.fitness},{neutral_percent}\n")
...
#     neutral_percent = measure_neutral_neighborhood_percentage(exp, genotype_sol)
